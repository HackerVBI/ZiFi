...
int main (int argc, char *argv[]) {
	...
	while ((argc > 1) && (argv[1][0] == '-' && argv[1][1] == '-')) {
		...
		if (strcmp (argv[1], "--debug") == 0) {
		    ...
		    if (argv[2] != NULL) {
			//printf("arg %d = param %s, param2 %s\n", argc, argv[1], argv[2]);
		
			//sxgBgColor = argv[2];
			sxgBgColor = (unsigned char)strtol(argv[2], NULL, 10);
		    } else {
			wrongParams = true;
		    }
		}
	...
	if (filename == NULL || wrongParams) {
		...
		printf("Commands:\n");
	  	...
	  	printf("  --bg nn   set background color. where nn number from palette (default #00)\n");
	} else {
						...
						if (realColorz > 0) {
							...
							if (bmpBits == 4 || bmpBits == 8) {
								...
								if ( dataLen > 0 ) {
									...
									unsigned long workWidth = bmpWidth;

									if (bmpBits == 4) {
										workWidth = bmpWidth / 2;
										if (bmpWidth % 2 >=5 ) {
										    workWidth++;
										}
										dataLen = ( workWidth * bmpHeigth );
									}
									...
									// int delta = workWidth % 4;
									int rowSize = (bmpBits * bmpWidth + 31) / 32 * 4;

									printf("Bitmap rowSize - %d bytes\n", rowSize);

									for (int y = bmpHeigth - 1; y >= 0; y-- ) {
									// for (int y = 0; y <bmpHeigth; y++ ) {
										for (int x = 0; x < workWidth; x++){

											unsigned char colorPixel = *(bmpImageData + x + ( y * rowSize));
									...
									// +#0000 #04 #7f+"SXG" - 4 байта сигнатура, что это формат файла
									// +#0004 #01 1 байт версия формата
									// +#0005 #01 1 байт цвет фона (для очистки)
									...
									if (writeSxgFile != NULL) {
										...
										unsigned char *sxgVer = "\x02";
										fwrite(sxgVer, 1, 1, writeSxgFile);

										//unsigned char *sxgBg = "\x00";
										fwrite(&sxgBgColor, 1, 1, writeSxgFile);

										unsigned char *sxgPack = "\x00";
										fwrite(sxgPack, 1, 1, writeSxgFile);
